...
To successfully run these scripts you'll need to have Xcode and Xcode command line tools (`xcode-select --install`) installed.
Additional to that the `sbhc.py` requires [Python bindings](https://pypi.python.org/pypi/clang/) for libclang matching OSx/macOS'
clang version (check with `clang --version`). You must install these bindings before running the script.
